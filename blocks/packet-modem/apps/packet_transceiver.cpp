...
int main(int argc, char** argv)
{
    ...
    if ((argc < 5) || (argc > 8)) {
        std::println(stderr,
                     ...
                     argv[0]);
        std::println(stderr, "");
        std::println(stderr, "the default sample rate is 3.2 Msps");
    ...
    auto& packet_type_filter = fg.emplaceBlock<gr::packet_modem::PacketTypeFilter<>>(
        gr::packet_modem::make_props({ { "packet_type", gr::packet_modem::pmt_value(std::string("user_data")) } }));
    ...
    auto& sink = fg.emplaceBlock<gr::packet_modem::TunSink>(
        gr::packet_modem::make_props({ { "tun_name", gr::packet_modem::pmt_value(std::string("gr4_tun_rx")) },
                                       { "netns_name", gr::packet_modem::pmt_value(std::string("gr4_rx")) } }));
