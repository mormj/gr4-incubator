...
int main(int argc, char* argv[])
{
    ...
    if (argc != 3) {
        std::println(stderr, "usage: {} output_file stream_mode", argv[0]);
        std::exit(1);
    }
    ...
    auto& sink =
        fg.emplaceBlock<gr::packet_modem::FileSink<c64>>(
            gr::packet_modem::make_props({ { "filename", gr::packet_modem::pmt_value(std::string(argv[1])) } }));
    ...
    if (!stream_mode) {
        ...
        auto& pdu_to_stream = fg.emplaceBlock<gr::packet_modem::PduToTaggedStream<c64>>(
            gr::packet_modem::make_props({ { "packet_len_tag_key", gr::packet_modem::pmt_value(std::string()) } }));
        if (max_in_samples) {
