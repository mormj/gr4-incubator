...
public:
    ...
    gr::work::Status processBulk(::gr::InputSpanLike auto& countSpan,
                                 ::gr::OutputSpanLike auto& outSpan)
    {
#ifdef TRACE
        std::println("{}::processBulk(countSpan.size() = {}, outSpan.size() = {}), "
...
#ifdef TRACE
            std::println("{} latency management region full", this->name);
#endif
            outSpan.publish(0);
        ...
        if (ret < 0) {
            throw gr::exception(std::format("select() failed: {}", strerror(errno)));
        } else if (ret == 0) {
...
#ifdef TRACE
                std::println("{} produced IDLE packet", this->name);
#endif
            } else {
                // timeout expired; return to the scheduler without a packet
#ifdef TRACE
                std::println("{} timeout expired; returning to scheduler", this->name);
#endif
                outSpan.publish(0);
                return gr::work::Status::OK;
            }
            ...
            if (ret < 0) {
                throw gr::exception(std::format("read failed: {}", strerror(errno)));
            }
...
#ifdef TRACE
            std::println("{} produced non-IDLE packet", this->name);
#endif
        }
