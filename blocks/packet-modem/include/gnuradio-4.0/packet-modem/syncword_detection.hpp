...
private:
    using c64 = std::complex<float>;
    using FFT = gr::algorithm::FFT<c64, c64>;
...
public:
                                 ...
                                 ::gr::OutputSpanLike auto& outSpan)
    {
        ...
        if (inSpan.size() < fft_size) {
            ...
            outSpan.publish(0);
#ifdef TRACE
            std::println("{} called with {} input items, but minimum is {}",
                         this->name,
                         inSpan.size(),
                         in.min_samples);
#endif
        ...
        for (j = 0; j + fft_size <= inSpan.size(); j += _stride) {
            auto fft_span = std::span<const c64>(inSpan.data() + j, fft_size);
            samples_fft = _fft.compute(fft_span, std::move(samples_fft));
            ...
            for (const size_t nfreq : std::views::iota(0UZ, num_freq_bins)) {
                ...
                _ifft_tmp = samples_fft_prod;
                for (auto& z : _ifft_tmp) {
                    z = std::conj(z);
                }
                correlation[nfreq] = _fft.compute(_ifft_tmp, std::move(correlation[nfreq]));
                for (auto& z : correlation[nfreq]) {
                    z = std::conj(z);
                }
            }
        ...
        outSpan.publish(j);
#ifdef TRACE
        std::println("{} consume & publish = {}", this->name, j);
#endif

        return gr::work::Status::OK;
    }

    GR_MAKE_REFLECTABLE(SyncwordDetection,
                        in,
                        out,
                        fft_size,
                        samples_per_symbol,
