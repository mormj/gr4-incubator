...
public:
                                 ...
                                 ::gr::OutputSpanLike auto& outSpan)
    {
        ...
        std::println("{}::processBulk(inSpan.size() = {}, outSpan.size = {}), "
                     "_data_remaining = {}, inputTagsPresent() = {}",
                     this->name,
                     inSpan.size(),
                     outSpan.size(),
                     _data_remaining,
        ...
        if (_data_remaining == 0) {
            ...
            _data_remaining = gr::packet_modem::pmt_cast<uint64_t>(len_it->second);
            _header_remaining = skip_header_bytes;
            if (_header_remaining >= _data_remaining) {
                std::println("{} WARNING: received packet shorter than header "
                             "(length {})",
                             this->name,
                             _data_remaining);
            }
...
#ifdef TRACE
        std::println("{}::processBulk() consume = {}, publish = {}",
                     this->name,
                     consumed,
                     published);
#endif
    ...
    GR_MAKE_REFLECTABLE(CrcAppend, in, out, num_bits, poly, initial_value, final_xor, input_reflected, result_reflected, swap_endianness, skip_header_bytes, packet_len_tag_key);
};
