...
#include <gnuradio-4.0/packet-modem/pdu_to_vector.hpp>
...
public:
                   ...
                   float syncword_threshold = 9.5)
    {
        ...
        auto& _syncword_detection = fg.emplaceBlock<SyncwordDetection>(gr::packet_modem::make_props({
            { "rrc_taps", gr::pmt::Value(gr::Tensor<float>(gr::data_from, rrc_taps)) },
            { "syncword", gr::pmt::Value(gr::Tensor<uint8_t>(gr::data_from, syncword)) },
            { "constellation", gr::pmt::Value(gr::Tensor<c64>(gr::data_from, bpsk_constellation)) },
            { "min_freq_bin", gr::pmt::Value(static_cast<int64_t>(-syncword_freq_bins)) },
            { "max_freq_bin", gr::pmt::Value(static_cast<int64_t>(syncword_freq_bins)) },
            { "power_threshold", gr::pmt::Value(syncword_threshold) },
        }));
        syncword_detection = &_syncword_detection;
        auto& syncword_detection_filter = fg.emplaceBlock<SyncwordDetectionFilter<>>(
            gr::packet_modem::make_props({ { "samples_per_symbol", gr::pmt::Value(static_cast<uint64_t>(samples_per_symbol)) } }));
        ...
        auto& freq_correction = fg.emplaceBlock<CoarseFrequencyCorrection<>>(
            gr::packet_modem::make_props(
                { { "delay", gr::pmt::Value(static_cast<uint64_t>((rrc_taps.size() - 1) / 2 + samples_per_symbol)) } }));
        ...
        auto& symbol_filter = fg.emplaceBlock<SymbolFilter<c64, c64, float>>(
            gr::packet_modem::make_props({
                { "taps", gr::pmt::Value(gr::Tensor<float>(gr::data_from, rrc_taps_pfb)) },
                { "num_arms", gr::pmt::Value(static_cast<uint64_t>(symbol_filter_pfb_arms)) },
                { "samples_per_symbol", gr::pmt::Value(static_cast<uint64_t>(samples_per_symbol)) },
                { "delay", gr::pmt::Value(static_cast<uint64_t>(rrc_taps.size() - 1)) },
            }));
        ...
        auto& syncword_wipeoff =
            fg.emplaceBlock<SyncwordWipeoff<>>(
                gr::packet_modem::make_props({
                    { "syncword", gr::pmt::Value(gr::Tensor<float>(gr::data_from, syncword_bipolar)) },
                }));
        auto& payload_metadata_insert =
            fg.emplaceBlock<PayloadMetadataInsert<>>(gr::packet_modem::make_props({
                { "log", gr::pmt::Value(log) },
            }));
        ...
        auto& constellation_decoder = fg.emplaceBlock<ConstellationLLRDecoder<>>(
            gr::packet_modem::make_props({
                { "noise_sigma", gr::pmt::Value(0.7f) },
                { "constellation", gr::pmt::Value(std::string("QPSK")) },
            }));
        auto& descrambler = fg.emplaceBlock<AdditiveScrambler<float>>(
            gr::packet_modem::make_props({
                { "mask", gr::pmt::Value(uint64_t{ 0x4001U }) },
                { "seed", gr::pmt::Value(uint64_t{ 0x18E38U }) },
                { "length", gr::pmt::Value(uint64_t{ 16U }) },
                { "reset_tag_key", gr::pmt::Value(std::string("header_start")) },
            }));
        auto& header_payload_split = fg.emplaceBlock<HeaderPayloadSplit<>>(
            gr::packet_modem::make_props({
                { "packet_len_tag_key", gr::pmt::Value(packet_len_tag_key) },
            }));
        ...
        auto& payload_pack =
            fg.emplaceBlock<PackBits<>>(gr::packet_modem::make_props({
                { "inputs_per_output", gr::pmt::Value(static_cast<uint64_t>(8UZ)) },
                { "bits_per_input", gr::pmt::Value(uint8_t{ 1 }) },
                { "packet_len_tag_key", gr::pmt::Value(packet_len_tag_key) },
            }));
        auto& _payload_crc_check = fg.emplaceBlock<CrcCheck<>>(
            gr::packet_modem::make_props({
                { "discard_crc", gr::pmt::Value(true) },
                { "packet_len_tag_key", gr::pmt::Value(packet_len_tag_key) },
            }));
        ...
        if (zmq_output) {
            auto& symbols_split = fg.emplaceBlock<HeaderPayloadSplit<c64>>(
                gr::packet_modem::make_props({
                    { "header_size", gr::pmt::Value(static_cast<uint64_t>(128UZ)) },
                    { "payload_length_key", gr::pmt::Value(std::string("payload_symbols")) },
                }));
            auto& header_to_pdu = fg.emplaceBlock<TaggedStreamToPdu<c64>>();
            auto& payload_to_pdu = fg.emplaceBlock<TaggedStreamToPdu<c64>>();
            auto& header_to_vec = fg.emplaceBlock<PduToVector<c64>>();
            auto& payload_to_vec = fg.emplaceBlock<PduToVector<c64>>();
            auto& zmq_header_sink =
                fg.emplaceBlock<gr::zeromq::ZmqPushSink<std::vector<c64>>>(
                    gr::packet_modem::make_props({
                    { "endpoint", gr::pmt::Value(std::string("tcp://*:5000")) },
                }));
            auto& zmq_payload_sink =
                fg.emplaceBlock<gr::zeromq::ZmqPushSink<std::vector<c64>>>(
                    gr::packet_modem::make_props({
                    { "endpoint", gr::pmt::Value(std::string("tcp://*:5001")) },
                }));
            ...
            if (fg.connect<"out">(header_to_pdu).to<"in">(header_to_vec) !=
                ConnectionResult::SUCCESS) {
                throw std::runtime_error(connection_error);
            }
            if (fg.connect<"out">(payload_to_pdu).to<"in">(payload_to_vec) !=
                ConnectionResult::SUCCESS) {
                throw std::runtime_error(connection_error);
            }
            if (fg.connect<"out">(header_to_vec).to<"in">(zmq_header_sink) !=
                ConnectionResult::SUCCESS) {
                throw std::runtime_error(connection_error);
            }
            if (fg.connect<"out">(payload_to_vec).to<"in">(zmq_payload_sink) !=
                ConnectionResult::SUCCESS) {
                throw std::runtime_error(connection_error);
            }
        }
