...
public:
    ...
    gr::work::Status processBulk(::gr::InputSpanLike auto& headerSpan,
                                 ...
                                 ::gr::OutputSpanLike auto& ignoredSpan)
    {
#ifdef TRACE
        std::println("{}::processBulk(headerSpan.size() = {}, ignoredSpan.size(), "
        ...
        if (this->inputTagsPresent()) {
            ...
            if (gr::packet_modem::has_prop(tag.map, syncword_amplitude_key)) {
#ifdef TRACE
                std::println("{} got {} tag", this->name, syncword_amplitude_key);
#endif
                auto get_float = [&](std::string_view key, float fallback) {
                    if (auto it = gr::packet_modem::find_prop(tag.map, key); it != tag.map.end()) {
                        return gr::packet_modem::pmt_cast<float>(it->second);
                    }
                    return fallback;
                };
                auto get_double = [&](std::string_view key, double fallback) {
                    if (auto it = gr::packet_modem::find_prop(tag.map, key); it != tag.map.end()) {
                        return gr::packet_modem::pmt_cast<double>(it->second);
                    }
                    return fallback;
                };
                auto get_int = [&](std::string_view key, int fallback) {
                    if (auto it = gr::packet_modem::find_prop(tag.map, key); it != tag.map.end()) {
                        return gr::packet_modem::pmt_cast<int>(it->second);
                    }
                    return fallback;
                };
                ...
                if (!_in_packet) {
                    ...
                    if (log) {
                        std::println(
                            ...
                            "time est = {:.4}",
                            _num_packet,
                            get_float("syncword_amplitude", 0.0f),
                            get_double("syncword_freq", 0.0),
                            get_int("syncword_freq_bin", 0),
                            get_float("syncword_esn0_db", 0.0f),
                            get_float("syncword_time_est", 0.0f));
                    }
                } else {
                    if (log) {
                        std::println("syncword received inside packet {}; ignoring. "
        ...
        while (out_item < outSpan.end() && in_item < inSpan.end()) {
            ...
            if (_position == syncword_size) {
                ...
                const gr::property_map header_map = {
                    ...
                    { header_start_key, gr::packet_modem::pmt_null() },
            ...
            if (_position == syncword_size + header_size && out_item < outSpan.end() &&
                ...
                if (header_item < headerSpan.end()) {
                    ...
                    if (gr::packet_modem::has_prop(meta, invalid_header_key)) {
                        // header decode failed; drop this packet
                        _in_packet = false;
                        in_item = inSpan.end(); // consume remaining input
                        ++header_item;
                        if (log) {
                            std::println("header decode failed for packet {}",
                                         _num_packet);
                        }
                        break;
                    }
                    const uint64_t packet_length =
                        gr::packet_modem::pmt_cast<uint64_t>(meta.at(packet_length_key));
                    if (packet_length == 0) {
                    ...
                    if (log) {
                        std::println(
                            ...
                            _num_packet,
                            packet_length,
                            gr::packet_modem::pmt_cast<std::string>(meta["packet_type"]));
                    }
...
#ifdef TRACE
        std::println("{} consumed = {}, published = {}",
                     this->name,
    ...
    GR_MAKE_REFLECTABLE(PayloadMetadataInsert, parsed_header, in, out, ignored_syncword, syncword_size, header_size, syncword_costas_loop_bandwidth, header_costas_loop_bandwidth, payload_costas_loop_bandwidth, log);
};
