...
public:
    ...
    {
#ifdef TRACE
        std::println("{} resetting LFSR", this->name);
#endif
        _reg = seed;
        _current_count = 0;
    }
    ...
    {
        if ((!reset_tag_key.empty() && this->inputTagsPresent() &&
             gr::packet_modem::has_prop(this->mergedInputTag().map, reset_tag_key)) ||
    ...
    GR_MAKE_REFLECTABLE(AdditiveScrambler, in, out, mask, seed, length, count, reset_tag_key);
};
