...
public:
    ...
    static inline gr::property_map _get_tags(gr::PortIn<T, gr::Async>& inPort)
    {
        gr::property_map map{};
        const auto position = inPort.streamReader().position();
        auto& tag_reader = inPort.tagReader();
        const auto tag_data = tag_reader.get();
    ...
    _samples_to_next_tag(gr::PortIn<T, gr::Async>& inPort, ssize_t offset)
    {
        auto& tag_reader = inPort.tagReader();
        const auto tag_data = tag_reader.get();
        if (tag_data.empty()) {
    ...
    {
#ifdef TRACE
        std::print("{}::processBulk(outSpan.size() = {}) ", this->name, outSpan.size());
        for (size_t j = 0; j < inSpans.size(); ++j) {
            std::print("inSpans[{}].size() = {} ", j, inSpans[j].size());
        }
        std::print("\n");
#endif
        ...
        if (_packet_lens.empty()) {
            ...
            for (auto& inPort : in) {
                ...
                std::println("map = {}", map);
#endif
                const auto len_it = gr::packet_modem::find_prop(map, packet_len_tag);
                if (len_it == map.end()) {
                    throw gr::exception("expected packet_len tag key not found");
                }
                _packet_lens.push_back(gr::packet_modem::pmt_cast<uint64_t>(len_it->second));
            }
            _current_input = 0;
        ...
        while (published < outSpan.size()) {
...
#ifdef TRACE
            std::println("{} _current_input = {}, samples_to_tag = {}, n = {}",
                         this->name,
                         _current_input,
                         samples_to_tag,
                         n);
            ...
            gr::packet_modem::erase_prop(map, packet_len_tag);
            if (!map.empty()) {
...
#ifdef TRACE
        std::println("{} outSpan.publish({})", this->name, published);
#endif
    ...
    GR_MAKE_REFLECTABLE(PacketMux, in, out, num_inputs);
};
