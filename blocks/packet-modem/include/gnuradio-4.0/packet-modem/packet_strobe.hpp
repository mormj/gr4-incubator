...
public:
                         ...
                         const gr::property_map& /* new_settings */)
    {
        _packet_len_tag =
            packet_len_tag_key.empty()
                ? std::nullopt
                : std::optional<gr::property_map>{gr::packet_modem::make_props(
                      { { packet_len_tag_key, gr::packet_modem::pmt_value(static_cast<uint64_t>(packet_len)) } })};
    }
    ...
    {
#ifdef TRACE
        std::println("{}::processBulk(outSpan.size() = {}), _position = {}, "
...
#ifdef TRACE
            std::println("{} publishing packet length tag (now = {})",
                         this->name,
                         ClockSourceType::now());
#endif
    ...
    GR_MAKE_REFLECTABLE(PacketStrobe, out, packet_len, interval_secs, packet_len_tag_key, sleep);
};
