...
private:
    ...
    {
#ifdef TRACE
        std::println("{}::set_phase({})", this->name, phase);
#endif
        _phase = phase;
        _freq = 0;
    }
...
public:
...
#ifdef TRACE
        std::println("{} k1 = {}, k2 = {}", this->name, k1, k2);
#endif
                                 ...
                                 ::gr::OutputSpanLike auto& outSpan)
    {
        ...
        if (this->inputTagsPresent()) {
            ...
            if (const auto it = gr::packet_modem::find_prop(tag.map, syncword_phase_key);
                it != tag.map.end()) {
                set_phase(gr::packet_modem::pmt_cast<TPhase>(it->second));
            }
        }
#ifdef TRACE
        std::println("{}::processBulk() constellation = {}, _phase = {}",
                     this->name,
                     constellation,
                     _phase);
#endif
    ...
    GR_MAKE_REFLECTABLE(CostasLoop, in, out, loop_bandwidth, constellation);
};
