...
public:
                                 ...
                                 ::gr::OutputSpanLike auto& outSpan)
    {
        ...
        if (this->inputTagsPresent()) {
            ...
            std::println("{} tags: index = {}, map = {}", this->name, tag.index, tag.map);
        }
#endif
        if (inSpan.size() == 0) {
            if (!inSpan.consume(0)) {
        ...
        if (_remaining == 0) {
            ...
            auto not_found_error = [this]() {
                this->emitErrorMessage(
                    ...
                    std::format("{} expected packet-length tag not found", this->name));
                this->requestStop();
            ...
            const auto it = tag.map.find(packet_len_tag_key);
            if (it == tag.map.end()) {
                return not_found_error();
            }
            _remaining = gr::packet_modem::pmt_cast<uint64_t>(it->second);
            if (_remaining == 0) {
                this->emitErrorMessage(
                    std::format("{}::processBulk", this->name),
                    std::format("{} received packet-length equal to zero", this->name));
                this->requestStop();
                return gr::work::Status::ERROR;
            }
            _pdu.data.clear();
        ...
        if (this->inputTagsPresent()) {
            ...
            if (_index == 0) {
                if (auto it = tag.map.find(packet_len_tag_key); it != tag.map.end()) {
                    tag.map.erase(it);
                }
            }
        ...
        if (_remaining == 0) {
#ifdef TRACE
            if (!_pdu.tags.empty()) {
                std::println("{} publishing PDU with tags:", this->name);
                for (const auto& tag : _pdu.tags) {
                    std::println("index = {}, map = {}", tag.index, tag.map);
                }
            }
#endif
            ...
            outSpan.publish(1);
#ifdef TRACE
            std::println("{} consume = {}, publish = 1", this->name, to_consume);
#endif
        } else {
            outSpan.publish(0);
...
#ifdef TRACE
            std::println("{} consume = {}, publish = 0, _remaining = {}, "
                         "_pdu.data.size() = {}",
                         this->name,
                         to_consume,
                         _remaining,
    ...
    GR_MAKE_REFLECTABLE(TaggedStreamToPdu, in, out, packet_len_tag_key);
};
