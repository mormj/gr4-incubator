...
public:
                                 ...
                                 ::gr::OutputSpanLike auto& countSpan)
    {
        ...
        std::println("{}::processBulk(inSpan.size() = {}, outSpan.size = {}, "
                     "countSpan.size = {}), "
                     "_remaining = {}, _packet_count = {}, inputTagsPresent = {}",
                     this->name,
                     inSpan.size(),
                     outSpan.size(),
                     countSpan.size(),
...
#ifdef TRACE
        std::println("{}::processBulk published = {} ", this->name, to_publish);
#endif

        return gr::work::Status::OK;
    }

    GR_MAKE_REFLECTABLE(PacketToStream, in, out, count, packet_len_tag_key);
};
