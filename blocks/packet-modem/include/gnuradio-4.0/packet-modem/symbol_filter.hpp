...
public:
                                 ...
                                 ::gr::OutputSpanLike auto& outSpan)
    {
        ...
        if (this->inputTagsPresent()) {
            ...
            if (const auto amp_it = gr::packet_modem::find_prop(tag.map, syncword_amplitude_key);
                amp_it != tag.map.end()) {
#ifdef TRACE
                ...
                _scale = 1.0f / gr::packet_modem::pmt_cast<float>(amp_it->second);

                // time_est is in the range [-0.5, 0.5]
                float time_est = 0.0f;
                if (const auto time_it = gr::packet_modem::find_prop(tag.map, syncword_time_est_key);
                    time_it != tag.map.end()) {
                    time_est = gr::packet_modem::pmt_cast<float>(time_it->second);
                }
                ...
                if (time_est < 0.0f) {
                    ...
                    float syncword_phase = 0.0f;
                    double syncword_freq = 0.0;
                    if (const auto phase_it = gr::packet_modem::find_prop(tag.map, "syncword_phase");
                        phase_it != tag.map.end()) {
                        syncword_phase = gr::packet_modem::pmt_cast<float>(phase_it->second);
                    }
                    if (const auto freq_it = gr::packet_modem::find_prop(tag.map, "syncword_freq");
                        freq_it != tag.map.end()) {
                        syncword_freq = gr::packet_modem::pmt_cast<double>(freq_it->second);
                    }
                    gr::packet_modem::set_prop(
                        tag.map,
                        "syncword_phase",
                        gr::packet_modem::pmt_value(static_cast<float>(
...
#ifdef TRACE
                        std::println("{} publishTag() {} at index = {}",
                                     this->name,
                                     _tags[0].map,
    ...
    GR_MAKE_REFLECTABLE(SymbolFilter, in, out, samples_per_symbol, taps, num_arms, delay);
};
