...
public:
    ...
    void open_tun()
    {
        ...
        if (!netns_name.empty()) {
            const auto path = std::format("/proc/self/task/{}/ns/net", gettid());
            original_netns = open(path.c_str(), O_RDONLY);
            if (original_netns < 0) {
                throw gr::exception(
                    std::format("could not open {}: {}", path, strerror(errno)));
            }

            const auto path_new = std::format("/var/run/netns/{}", netns_name);
            ...
            if (setns(new_ns, CLONE_NEWNET) < 0) {
                throw gr::exception(std::format("setns() failed: {}", strerror(errno)));
            };
            close(new_ns);
        }
        ...
        if (_tun_fd < 0) {
            throw gr::exception(
                std::format("could not open /dev/net/tun: {}", strerror(errno)));
        }
        struct ifreq ifr;
        ...
        if (ioctl(_tun_fd, TUNSETIFF, static_cast<void*>(&ifr)) < 0) {
            throw gr::exception(std::format("ioctl failed: {}", strerror(errno)));
        }
    }
