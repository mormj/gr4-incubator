...
public:
                                 ...
                                 ::gr::OutputSpanLike auto& outSpan)
    {
        ...
        if (outSpan.size() == 0) {
#ifdef TRACE
            std::println("{}::processBulk returning INSUFFICIENT_OUTPUT_ITEMS",
                         this->name);
#endif
            std::ignore = inSpan.consume(0);
            outSpan.publish(0);
        ...
        if (inSpan.size() == 0) {
#ifdef TRACE
            std::println("{}::processBulk returning INSUFFICIENT_INPUT_ITEMS",
                         this->name);
#endif
            std::ignore = inSpan.consume(0);
            outSpan.publish(0);
    ...
    GR_MAKE_REFLECTABLE(Head, in, out, num_items);
};
