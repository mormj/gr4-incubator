...
These instructions target the legacy `gr4-packet-modem` images. In this repo,
the packet-modem module lives under `blocks/packet-modem` inside the
`gr4-incubator` workspace. Prefer the top-level container/devcontainer here when
available.
...
docker run --rm --net host -e TERM -e HOME=/home/ubuntu \
    ...
    -v $HOME/gr4-incubator:/workspaces/gr4-incubator \
    --user ubuntu \
    --entrypoint bash \
    -it ghcr.io/daniestevez/gr4-packet-modem
```

This assumes that `$HOME/gr4-incubator` contains a checkout of this repository,
which is made available to the docker container at `/workspaces/gr4-incubator`.
It also assumes that `$UID` is 1000. This is the UID of the user `ubuntu` in the
container, so if there is a UID mismatch there will be file permission issues
when sharing the checkout between the container and the host.
...
In this case, the `$HOME/gr4-incubator` checkout is also shared with the
