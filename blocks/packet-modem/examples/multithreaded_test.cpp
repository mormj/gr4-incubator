...
int main(int argc, char* argv[])
{
    ...
    auto& mux = fg.emplaceBlock<PacketMux<Pdu<T>>>(
        make_props({ { "num_inputs", pmt_value(mux_num_inputs) } }));
    ...
    for (const auto j : std::views::iota(0UZ, mux_num_inputs)) {
        ...
        auto& to_tagged = fg.emplaceBlock<StreamToTaggedStream<T>>(
            make_props({ { "packet_length", pmt_value(packet_length) } }));
        ...
        expect(fatal(eq(gr::ConnectionResult::SUCCESS,
                        fg.connect(to_pdu, "out"s, mux, std::format("in#{}", j)))));
    }
