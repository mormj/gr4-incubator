...
public:
    ...
    {
        std::println("Process<T>::processBulk(inSpan.size(), outSpan.size() = {})",
                     inSpan.size(),
                     outSpan.size());
    ...
    GR_MAKE_REFLECTABLE(Process, in, out);
};
...
public:
    ...
    gr::work::Status processBulk(::gr::InputSpanLike auto& inSpan,
                                 ::gr::OutputSpanLike auto& outSpan)
    {
        std::println("Process<Pdu<T>>::processBulk(inSpan.size(), outSpan.size() = {})",
                     inSpan.size(),
                     outSpan.size());
        const auto n = std::min(inSpan.size(), outSpan.size());
        for (size_t j = 0; j < n; ++j) {
