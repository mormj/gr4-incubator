...
boost::ut::suite CrcTests = [] {
    ...
    "crc_append_one_packet"_test = [](size_t packet_len) {
        ...
        const std::vector<Tag> t = {
            { 0, make_props({ { "packet_len", pmt_value(packet_len) } }) }
        };
        auto& source = fg.emplaceBlock<VectorSource<uint8_t>>();
        source.data = v;
        source.tags = t;
        auto& crc_append = fg.emplaceBlock<CrcAppend<>>(
            make_props({ { "num_bits", pmt_value(16U) },
                         { "poly", pmt_value(uint64_t{ 0x1021 }) },
                         { "initial_value", pmt_value(uint64_t{ 0xFFFF }) },
                         { "final_xor", pmt_value(uint64_t{ 0xFFFF }) },
                         { "input_reflected", pmt_value(true) },
                         { "result_reflected", pmt_value(true) } }));
        ...
        expect(tags[0].map == make_props({ { "packet_len", pmt_value(packet_len + 2U) } }));
    ...
    "crc_check_one_packet"_test =
        [](auto args) {
            ...
            const std::vector<Tag> t = {
                { 0, make_props({ { "packet_len", pmt_value(packet_len + 2U) } }) }
            };
            auto& source = fg.emplaceBlock<VectorSource<uint8_t>>();
            source.data = v;
            source.tags = t;
            ...
            expect(tag.map == make_props({ { "packet_len", pmt_value(data.size()) } }));
        } |
