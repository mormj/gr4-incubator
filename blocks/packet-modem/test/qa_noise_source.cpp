...
boost::ut::suite NoiseSourceTests = [] {
    ...
    "noise_source_gaussian_power"_test = []<typename T> {
        ...
        auto& source = fg.emplaceBlock<NoiseSource<T>>(
            make_props({ { "noise_type", pmt_value("gaussian") },
                         { "amplitude", pmt_value(amplitude) } }));
        auto& head =
            fg.emplaceBlock<Head<T>>(
                make_props({ { "num_items", pmt_value(static_cast<size_t>(num_items)) } }));
