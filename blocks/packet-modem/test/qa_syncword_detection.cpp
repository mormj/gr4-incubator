    ...
    "syncword_detection"_test = [](float freq_error) {
        ...
        auto& constellation_mapper =
            fg.emplaceBlock<Mapper<uint8_t, c64>>(
                make_props({ { "map", pmt_value(constellation) } }));
        ...
        auto& rrc_interp = fg.emplaceBlock<InterpolatingFirFilter<c64, c64, float>>(
            make_props({ { "interpolation", pmt_value(samples_per_symbol) },
                         { "taps", pmt_value(rrc_taps) } }));
        auto& rotator = fg.emplaceBlock<gr::packet_modem::Rotator<>>(
            make_props({ { "phase_incr", pmt_value(freq_error) } }));
        auto& syncword_detection = fg.emplaceBlock<SyncwordDetection>(
            make_props({ { "rrc_taps", pmt_value(rrc_taps) },
                         { "syncword", pmt_value(syncword) },
                         { "constellation", pmt_value(constellation) },
                         { "min_freq_bin", pmt_value(-4) },
                         { "max_freq_bin", pmt_value(4) },
                         // set a high power threshold to avoid false detections
                         { "power_threshold", pmt_value(20.0f) } }));
        ...
        for (size_t j = 0; j < tags.size(); ++j) {
            ...
            const auto syncword_amplitude =
                gr::packet_modem::pmt_cast<float>(meta.at("syncword_amplitude"));
            expect(syncword_amplitude < 1.01f);
            expect(syncword_amplitude > 0.95f);
            const auto syncword_esn0_db = gr::packet_modem::pmt_cast<float>(meta.at("syncword_esn0_db"));
            expect(syncword_esn0_db >= 30.0f);
            const auto syncword_freq = gr::packet_modem::pmt_cast<float>(meta.at("syncword_freq"));
            expect(std::abs(syncword_freq - freq_error) < 5e-4f);
            const auto syncword_freq_bin = gr::packet_modem::pmt_cast<int>(meta.at("syncword_freq_bin"));
            ...
            const auto syncword_noise_power =
                gr::packet_modem::pmt_cast<float>(meta.at("syncword_noise_power"));
            expect(syncword_noise_power < 5e-4f);
            const auto syncword_phase = gr::packet_modem::pmt_cast<float>(meta.at("syncword_phase"));
            if (freq_error == 0.0f) {
                expect(std::abs(syncword_phase) < 1e-6f);
            }
            const auto syncword_time_est =
                gr::packet_modem::pmt_cast<float>(meta.at("syncword_time_est"));
            expect(std::abs(syncword_time_est) < 0.05f);
        }
        for (const auto& tag : tags) {
            std::println("tag {} {}", tag.index, tag.map);
        }
