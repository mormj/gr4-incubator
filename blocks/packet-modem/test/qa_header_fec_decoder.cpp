...
boost::ut::suite HeaderFecDecoderTests = [] {
    ...
    "header_fec_decoder"_test = [] {
        ...
        auto& unpack = fg.emplaceBlock<UnpackBits<>>(
            make_props({ { "outputs_per_input", pmt_value(8UZ) } }));
        auto& to_llr = fg.emplaceBlock<Mapper<uint8_t, float>>(
            make_props({ { "map", pmt_value(std::vector<float>{ 1.0f, -1.0f }) } }));
    ...
    "header_fec_decoder_invalid_codeword"_test = [] {
        ...
        for (size_t j = 0; j < tags.size(); ++j) {
            ...
            expect(tags[j].map ==
                   make_props({ { "invalid_header", gr::packet_modem::pmt_null() } }));
        }
    };
};
