...
boost::ut::suite VectorSourceAndSinkTests = [] {
    ...
    "vector_source_fixed_data"_test = [] {
        ...
        const std::vector<Tag> tags = {
            { 0, make_props({ { "a", gr::packet_modem::pmt_null() } }) },
            { 10, make_props({ { "b", pmt_value(0.1234) }, { "c", pmt_value(12345U) } }) },
            { 73, make_props({ { "d", pmt_value(std::vector<int>{ 1, 2, 3 }) },
                               { "e", pmt_value(0.0f) } }) },
            { std::ssize(v) - 1, make_props({ { "f", gr::packet_modem::pmt_null() } }) }
        };
    ...
    "vector_source_repeat"_test = [] {
        ...
        std::iota(v.begin(), v.end(), 0);
        const std::vector<Tag> tags = {
            { 0, make_props({ { "begin", gr::packet_modem::pmt_null() } }) },
            { std::ssize(v) - 1, make_props({ { "end", gr::packet_modem::pmt_null() } }) }
        };
        ...
        auto& head = fg.emplaceBlock<Head<int>>(
            make_props({ { "num_items",
                           pmt_value(static_cast<uint64_t>(num_items) * repetitions) } }));
        auto& sink = fg.emplaceBlock<VectorSink<int>>();
    ...
    "random_source"_test = [] {
        ...
        auto& source = fg.emplaceBlock<RandomSource<int>>(
            make_props({ { "minimum", pmt_value(static_cast<int>(min)) },
                         { "maximum", pmt_value(static_cast<int>(max)) },
